...
- [ ] **MEMSEC-04**: Retrieval and use authorization is enforced before every memory search, multi-search, context-pack assembly, ChatGPT action, export, summarization, agent dispatch, FTS/vector/graph index write, and derived evidence bundle. The policy decision must check actor identity, role, capability, tenant/project, purpose, source freshness, and security labels, then return allow, deny, redact, or review-required.
